...
<div class="container">
    <div id="content">	
                ...
                @foreach($productCarts->items as $key => $value)
                    <tr class="cart_item">
                        <td class="product-name">
                            <div class="media">
                                <img class="pull-left" src="{{ asset('assets/dest/images/products/'.$value['item']['image']) }}" width="50px" height="50px" alt="">
                        ...
                        <td class="product-price">
                            <span class="amount">$
                                ...
                                @else
                                {{$value['item']['promotion_price'] }}</span>
                                @endif
                        </td>
                        ...
                        <td class="product-quantity d-flex">
                            <form action="{{ route('cart.reduce', ['id' =>$value['item']['id']]) }}" method="POST">
                                @csrf
                                @method('PATCH')
                                <button  >-</button>
                            </form>
                            <label>{{ $value['qty'] }}</label>
                            <form action="{{ route('cart.increase', ['id' =>$value['item']['id']]) }}" method="POST">
                                @csrf
                                @method('PATCH')
                                <button >+</button>
                            </form>
                        ...
                        <td class="product-remove">
                            <form action="{{ route('cart.destroy', ['id' =>$value['item']['id']]) }}" method="POST">
                                @csrf
                                @method('DELETE')
        ...
        @else
        <label class="">Giỏ hàng rỗng !!!</label>
    @endif
        ...
        <div class="cart-collaterals">
            ...
            <div class="cart-totals pull-right">
                ...
                <div class="cart-totals-row"><span>Cart Subtotal:</span> <span>${{ number_format($productCarts->totalPrice) }}</span></div>
                <div class="cart-totals-row"><span>Shipping:</span> <span>Free Shipping</span></div>
                <div class="cart-totals-row"><span>Order Total:</span> <span>${{ number_format($productCarts->totalPrice) }}</span></div>
            </div>
        @endif
